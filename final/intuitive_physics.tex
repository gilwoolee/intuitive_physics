...
TODO: This equation is confusing, just use $M_\phi$ instead of $\mathcal{M}$ or $\phi$. The second term $P(\phi_\mathcal{M})$ should be $P_{t-1}(M_\phi)$, the prior belief over $M_\phi$, and what we're computing is the posterior distribution, $P_t(M_\phi)$.
...
\For{i = 1, 2, ..., $n_{itr}$}
  ...
  \EndFor
\EndFor
\State Update policy: $\pi(\theta_{i+1}) = BatchPolOpt(\theta_i, \{\tau_1, ..., \tau_{n_{sample}}\})$
\end{algorithmic}
\end{algorithm}
