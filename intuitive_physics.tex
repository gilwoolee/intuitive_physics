...
We make a simplification to the BARL formulation. We assume that $\phi$ is latent but constant, or that the rate of change for $\phi$ is slow enough that approximating the long-term value with a determinized $\phi$ is a reasonable short-term approximation for choosing one-step action, i.e. we can treat $V^*(s_t, \phi_t) \approx V^*(s_t, \phi_{t:\infty})$ for the purpose of one-step Bellman update.
