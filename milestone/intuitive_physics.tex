...
\begin{itemize}
\item[\checkmark] April 30 -- Set up all simulation environments in DART~\cite{lee2018dart} (including a kinematic simulation for the RACECAR)
...
The RACECAR is controlled by torque on the front wheels, and position and velocity inputs over the steering angle. The goal of the task is to track a given trajectory while minimizing the time of traversal. The physics parameters we plan to change are mass of the car and friction coefficient between the car and the road, both of which affect the vehicle dynamics significantly.
